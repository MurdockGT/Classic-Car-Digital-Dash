...
void loop() {
  ...
  double batteryVoltage = getBattVoltage(); 
  int currentRPM = getRPM();
  //choke();  my choke is broken physically
...
//delay(500);
 //  Transfer information to display arduino/4ds device
  ...
  Wire.write((byte)batteryVoltage);
  Wire.write((byte)fanStatus1);
...
// read the input on analog pin 
  ...
  float voltage = val * (5.00 / 1023.0);    // change for differant base voltage
  float tempr;
  if (voltage >= array[0])
  {
    tempr = array[1];
